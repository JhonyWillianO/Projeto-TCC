...
if (tempo_entre_atacas <= 0) {
    if (tiros_disparados < quantidade_tiros) {
        if (tempo_tiro <= 0) {
            ...
            tempo_tiro = 5; // Intervalo entre os tiros (ajuste conforme necessário)
        } else {
        ...
        tempo_entre_atacas = 6 * room_speed; // Define o próximo intervalo de ataque
    }
} else {
    ...
    if (movendo) {
        ...
        if (y > room_height - sprite_height / 2) {
            y = room_height - sprite_height / 2; // Corrige a posição para dentro da tela
            direcao_vertical *= -1; // Inverte a direção vertical
        } else if (y < sprite_height / 2) {
            y = sprite_height / 2; // Corrige a posição para dentro da tela
            direcao_vertical *= -1; // Inverte a direção vertical
        }
    }
}
